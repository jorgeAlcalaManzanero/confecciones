...
class VI_WOO_CART_ALL_IN_ONE_Frontend_MiniCart {
	...
	public function __construct() {
		...
		add_action( 'wp_ajax_woo_cart_all_in_one_select_cart_icon', array( $this, 'woo_cart_all_in_one_select_cart_icon', ) );
		add_action( 'wp_ajax_woo_cart_all_in_one_cart_icon_default_style', array( $this, 'woo_cart_all_in_one_select_cart_icon', ) );
		add_action( 'wp_ajax_woo_cart_all_in_oneremove_icon_style', array( $this, 'woo_cart_all_in_oneremove_icon_style', ) );
		add_action( 'wp_ajax_woo_cart_all_in_one_loading', array( $this, 'woo_cart_all_in_one_loading' ) );

		add_action( 'wp_ajax_woo_cart_all_in_one_menu_cart_icon', array( $this, 'woo_cart_all_in_one_menu_cart_icon', ) );
		add_action( 'wp_ajax_woo_cart_all_in_one_menu_text_type', array( $this, 'woo_cart_all_in_one_menu_text_type', ) );
	}
	...
	public function set_menu_cart_text( $style, $total, $pro_count = null ) {
		if ( empty( $pro_count ) ) {
			$pro_count = is_admin() ? 0 : WC()->cart->get_cart_contents_count();
		}
		if ( $total === 'total' ) {
			$total = is_admin() ? wc_price( 0 ) : WC()->cart->get_total();
		} else {
			$total = is_admin() ? wc_price( 0 ) : WC()->cart->get_cart_subtotal();
		}
		$menu_cart_one_text = '';
...
//				'menu_cart_enable_device'      => esc_attr( $this->get_params( 'menu_cart_enable_device' ) ),
	...
	public function create_menu_cart( $items, $args ) {
		...
		if ( empty( $mini_cart_menu_enable_pages )) {
			return $items;
		}
		$menu_data = array(
		...
		$items_menu= isset($args->menu) ?$args->menu: null;
		$items_id= $items_menu && isset($items_menu->term_id) ? $items_menu->term_id : null;
		if (empty($items_menu) || empty($items_id) || !in_array($items_id, $menu_data['menu_id'])){
			$check_item_enable = false;
			if ($items_menu && empty($items_id)){
				foreach ($menu_data['menu_id'] as $id){
					$t=wp_get_nav_menu_object($id);
					$t_slug= $t && isset($t->slug) ? $t->slug : null;
					if ($t_slug == $items_menu){
						$check_item_enable = true;
						break;
					}
				}
            }
			if (!$check_item_enable){
				return $items;
			}
        }
		...
		$navigation_title  = ( $menu_data['navigation_page'] == 1 ) ? __( 'View your shopping cart', 'woo-cart-all-in-one' ) : __( 'Quick checkout', 'woo-cart-all-in-one' );
		$vicaio_menu_class = is_admin() ? 'vi_wcaio_menu_cart' : 'vi_wcaio_menu_cart vi_wcaio_menu_cart_hidden';
		$check_vicaio_menu = strstr( $items, 'vi_wcaio_menu_cart' );
		if ( $check_vicaio_menu === false ) {
			ob_start();
			?>
            <li class="<?php echo esc_attr( $vicaio_menu_class ); ?>">
                <a href="<?php echo esc_attr( esc_url( $navigation_url ) ); ?>" title="<?php esc_attr_e( $navigation_title ); ?>" class="vi_wcaio_menubar_dropdowns">
                            <span class="vi_wcaio_mini_cart_menu_icon">
                                <?php echo wp_kses( $this->set_cart_icon_default_style( $menu_data['menu_cart_icon'] ), VI_WOO_CART_ALL_IN_ONE_DATA::extend_post_allowed_html() ); ?>
                            </span>
					<?php
					if ( $menu_data['menu_style'] ) {
						$menu_cart_one_text = $this->set_menu_cart_text( $menu_data['menu_cart_style_one_text'], $total, $pro_count );
						?>
                        <span class="vi_wcaio_menu_cart_text_one"><?php echo wp_kses( $menu_cart_one_text, VI_WOO_CART_ALL_IN_ONE_DATA::extend_post_allowed_html() ); ?></span>
						<?php
					}
					?>
                </a>
				<?php
				echo wp_kses( $this->create_mini_cart_content_menubar_dropdown(), VI_WOO_CART_ALL_IN_ONE_DATA::extend_post_allowed_html() );
				?>
            </li>
			<?php
			$html  = ob_get_clean();
                            ...
                            <div class="vi_wcaio_coupon">
                                ...
                                <button type="submit" class="button vi_wcaio_input_coupon-button" name="apply_coupon"
                                        value="<?php esc_attr_e( 'Apply coupon',
									        'woo-cart-all-in-one' ); ?>"><?php esc_html_e( 'Apply',
                ...
                <div class="vi_wcaio_mini_cart_sidebar_coupon-total">
                    ...
                    <div class="vi_wcaio_mini_cart_sidebar_total-subtotal">
                        ...
                        <div class="vi_wcaio_mini_cart_sidebar_subtotal">
                            <div><?php echo wp_kses_post( __( 'Subtotal:&nbsp; ', 'woo-cart-all-in-one' ) ); ?></div>
                    ...
                    <div class="vi_wcaio_mini_cart_sidebar-button"
                        ...
                        <input type="button" class="hidden"
                               value=" <?php esc_attr_e( 'Update Cart', 'woo-cart-all-in-one' ) ?>"
                        ...
                        <a href="<?php echo esc_attr( esc_url( get_permalink( wc_get_page_id( 'checkout' ) ) ) ); ?> ">
                            <input type="button"
...
//					$product_permalink = $product->get_permalink();
					?>
                    <li class="vi_wcaio_sidebar_product" data-id="<?php echo esc_attr( $product_id ); ?>"
                        ...
                        <div class="vi_wcaio_sidebar_product_img">
							...
							if ( empty( $product_permalink ) ) {
								...
								echo '<a href="' . esc_attr( esc_url( $product_permalink ) ) . '">' . $product_thumbnail . '</a>';
							}
							?>
                        </div>
                        <div class="vi_wcaio_sidebar_product_info">
                            <div class="vi_wcaio_sidebar_product-name-product">
								...
								if ( empty( $product_permalink ) ) {
									...
									echo '<a href="' . esc_attr( esc_url( $product_permalink ) ) . '">' . $product_name . '</a>';
								}
								?>
                            ...
                            <div class="vi_wcaio_sidebar_product-info-product">
                                ...
                                <div class="vi_wcaio_sidebar_product-number-product">
									<?php
									do_action( 'vi_wcaio_before_add_to_cart_quantity' );
									...
									do_action( 'vi_wcaio_after_add_to_cart_quantity' );
									?>
                                </div>
                            </div>
                        </div>
                        <div class="vi_wcaio_sidebar_product-delete-product">
							...
							echo sprintf( '<a href="%s" class="vi_wcaio_sidebar_product-remove" aria-label="%s" data-product_id="%s" data-cart_item_key="%s" data-product_sku="%s">'
								...
								esc_attr__( 'Remove', 'woo-cart-all-in-one' ),
								esc_attr( $product_id ),
			...
			echo esc_html__( 'No products in the cart.', 'woo-cart-all-in-one' );
			echo '</li>';
		}
	}
